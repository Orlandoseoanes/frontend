...
export default function Game() {
  ...
  useEffect(() => {
    ...
    if (!t) {
      ...
      const getWinners = async () => {
        try {
          const response = await axios.get("https://back-zboz.onrender.com/api/v1/winners", {
          headers: {
  ...
  const handleWinner = async (w) => {
    ...
    try {
      const t = localStorage.getItem("jwt-token");
      const current_winner = await axios.post("https://back-zboz.onrender.com/api/v1/winners", winner, {
        headers: {
          Authorization: `Bearer ${t}`,
        },
      });
